            ...
            if (_nf.dis_map.size() > 0) {
                ...
                for (auto itj : _nf.dis_map) {
                    ...
                    if (sf.has_node(_idj) && sf.id2nodeframe.at(_idj).vo_available) {
                        ...
                        if (fabs(ceres::asin(dheight/dis)) > params.distance_measurement_outlier_elevation_threshold ||
                            !enable_distance) {
            ...
            for (auto itj : _nf.dis_map) {
                ...
                if (sf.has_node(_idj) && sf.id2nodeframe.at(_idj).vo_available) {
                    ...
                    if (fabs(dis - est_dis) > params.distance_measurement_outlier_threshold ||
                        ...
                        fabs(ceres::asin(dheight/dis)) > params.distance_measurement_outlier_elevation_threshold ||
                        !enable_distance) {
                        printf("is outlier or distance is disable");
                        _nf.outlier_distance[_idj] = true;
                    } else {
...
//        if (solve_count % 10 == 0)
    ...
    if (num_res_blks > 1) {
        equv_cost = ceres::sqrt(equv_cost) / problem.NumResiduals() / sliding_window_size();
    }
